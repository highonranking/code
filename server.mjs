...
// Share a project with a simple name (e.g., /api/share/abhinav)
app.post('/api/share/:shareName', (req, res) => {
  ...
  // Build share URL - in dev mode, use port 5173 (Vite), in production use actual host
  let shareUrl;
  if (process.env.NODE_ENV === 'production') {
    shareUrl = `${req.protocol}://${req.get('host')}/s/${shareName}`;
  } else {
    // In dev mode, requests come through Vite proxy, so use 5173
    const host = req.get('host')?.split(':')[0] || 'localhost';
    shareUrl = `${req.protocol}://${host}:5173/s/${shareName}`;
  }
...
// Get shared project data via API
app.get('/api/s/:shareName', (req, res) => {
  const { shareName } = req.params;
...
// Serve the React app for shared links (only in production)
// In development, Vite handles this
if (process.env.NODE_ENV === 'production') {
  app.get('/s/:shareName', (req, res) => {
    const { shareName } = req.params;
    const project = sharedProjects.get(shareName);
    ...
    // Serve the React app with the share name in URL
...
app.delete('/api/s/:shareName', (req, res) => {
  const { shareName } = req.params;
...
app.get('/share/:shareName', (req, res) => {
  const { shareName } = req.params;
